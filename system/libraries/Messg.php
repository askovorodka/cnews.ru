...
class Messg {
    ...
    function fetch_lists($which='')
    {
    	...
    	$query = $this->DB->query("SELECT l.*, m.username, m.screen_name, m.member_id 
    ...
    function fetch_folders()
    {
    	...
    	$query = $this->DB->query("SELECT * FROM exp_message_folders WHERE member_id = '{$this->member_id}'");

    	if ($query->num_rows() == 0)
    	{
    		$this->DB->insert('exp_message_folders', array('member_id' => $this->member_id));
    	...
    	{	
    		...
    		$results = $this->DB->query("SELECT COUNT(*) AS count, message_folder FROM exp_message_copies 
    ...
    {	
    	...
    	for($i=1, $data = array(); $i <= $this->max_folders; $i++)
    	{
    		...
    		{
    			$data['folder'.$i.'_name'] = '';
    			$this->DB->query("UPDATE exp_message_copies SET message_deleted = 'y' 
    	...
    	$this->DB->update('exp_message_folders', $data, "member_id = '{$this->member_id}'");
    ...
    function abstract_menu()
    {
    	...
    	$query = $this->DB->query("SELECT COUNT(*) AS count FROM exp_message_data 
    	...
    	$query = $this->DB->query("SELECT COUNT(*) AS count FROM exp_message_copies 
    						 WHERE recipient_id = '{$this->member_id}'
    ...
    function drafts()
    {
		...
		$query = $this->DB->query("SELECT COUNT(exp_message_data.message_id) AS count ".$sql);
    ...
    function view_folder($folder_id='')
    {
		...
		$query = $this->DB->query("SELECT COUNT(exp_message_copies.copy_id) AS count ".$sql);
		...
		{
			$this->DB->query("UPDATE exp_message_copies SET message_received = 'y' 
						WHERE recipient_id = '{$this->member_id}'
...
DOH;
    ...
    function member_search($message='')
    {
        ...
        $query = $this->DB->query("SELECT group_id, group_title FROM exp_member_groups WHERE site_id = '".$this->DB->escape_str($PREFS->ini('site_id'))."' AND include_in_memberlist = 'y' ORDER BY group_title");
    ...
    {  
        ...
        foreach ($_POST as $key => $val)
		{
			...
			if ($key == 'group_id')
			{
				if ($val != 'any')
				{
					$search_query[] = " exp_member_groups.group_id ='".$this->DB->escape_str($_POST['group_id'])."'";
				}
			}
			else
			{
				if ($val != '')
				{
					$search_query[] = $key." LIKE '%".$this->DB->escape_str($val)."%'";
				}
			}
		}
        ...
        $sql = "SELECT DISTINCT exp_members.screen_name FROM exp_members, exp_member_groups 
        		...
        		AND exp_member_groups.site_id = '".$this->DB->escape_str($PREFS->ini('site_id'))."' AND include_in_memberlist = 'y'
        		AND ".$Q;                 
    ...
    {  
        ...
        foreach ($_POST as $key => $val)
		{
			...
			if ($key == 'group_id')
			{
				if ($val != 'any')
				{
					$search_query[] = " group_id ='".$this->DB->escape_str($_POST['group_id'])."'";
				}
			}
			else
			{
			...
			if (strstr($key, 'toggle') AND ! is_array($val))
			{
				...
				if ($IN->GBL('daction', 'POST') == 'delete')
				{
					if (substr($val, 0, 1) == 'd')
					{
						...
						$this->DB->query("DELETE FROM exp_message_data
        							WHERE sender_id = '{$this->member_id}'
        							AND message_id = '".$this->DB->escape_str(substr($val, 1))."'
        							AND message_status = 'draft'");
					}
					else
					{
						$this->DB->query("UPDATE exp_message_copies SET message_deleted = 'y', message_read = 'y'
        							WHERE recipient_id = '{$this->member_id}'
        							AND copy_id = '".$this->DB->escape_str($val)."'");
						...
						{
							$this->DB->query("UPDATE exp_members SET private_messages = private_messages - 1
        		...
        		elseif ($IN->GBL('moveto', 'POST') && $IN->GBL('moveto', 'POST') != 'none')
				{
					...
					{
						$this->DB->query("UPDATE exp_message_copies 
									SET message_deleted = 'n',
									message_folder = '".$this->DB->escape_str($folder_id)."'
        						    WHERE recipient_id = '{$this->member_id}'
        						    AND copy_id = '".$this->DB->escape_str($val)."'");
					}
				}
				elseif ($IN->GBL('copyto', 'POST') && $IN->GBL('copyto', 'POST') != 'none')
				{
					...
					if (is_numeric($folder_id) && $folder_id <= $this->max_folders && isset($this->folders[$folder_id]))
					{
						$query = $this->DB->query("SELECT * FROM exp_message_copies 
											 WHERE recipient_id = '{$this->member_id}'
        						    		 AND copy_id = '".$this->DB->escape_str($val)."'");
        						    		 
        				$query->row['copy_id'] = '';
        				$query->row['message_folder']  = $this->DB->escape_str($folder_id);
        				$query->row['message_deleted'] = 'n';
        				
        				$this->DB->insert('exp_message_copies', $query->row);
					}
				}
			}
		}
    ...
    function erase($delete='')
    {
    	...
    	{
    		$query = $this->DB->query("SELECT message_id, copy_id FROM exp_message_copies 
    				        	 WHERE recipient_id = '{$this->member_id}'
    	...
    	if ($query->num_rows > 0)
    	{
    		...
    		foreach($query->result as $row)
    		{
    			...
    			*/
    			...
    			$results = $this->DB->query("SELECT COUNT(*) AS count FROM exp_message_copies
    								   WHERE message_id = '".$this->DB->escape_str($row['message_id'])."'
    		...
    		{
    			$this->DB->query("DELETE FROM exp_message_data WHERE message_id IN ('".implode("','", $message_ids)."')");
    			
    			// Remove any attachments as well
    			
    			$query = $this->DB->query("SELECT attachment_location FROM exp_message_attachments WHERE message_id IN ('".implode("','", $message_ids)."')");
				...
				if ($query->num_rows > 0)
				{
					...
					$this->DB->query("DELETE FROM exp_message_attachments WHERE message_id IN ('".implode("','", $message_ids)."')");			
				}
    		}
    	
    		$this->DB->query("DELETE FROM exp_message_copies WHERE copy_id IN ('".implode("','", $copy_ids)."')");		
    	}
    	...
    	$results = $this->DB->query("UPDATE exp_members SET private_messages = '".$this->DB->escape_str($query->row['count'])."'
    						   WHERE member_id = '{$this->member_id}'");
    ...
    function convert_recipients($str, $type='string', $by='screen_name')
    {
    	...
    	for($i=0, $s = sizeof($temp_list); $i < $s; ++$i)
    	{
    		if(trim($temp_list[$i]) != '')
    		{
    			$list[] = $this->DB->escape_str($temp_list[$i]);
    		}
    	}
    	...
    	{
    		$query = $this->DB->query("SELECT {$by} FROM exp_members 
	...
	function _check_overflow($id)
	{
    	...
    	$sql = "SELECT exp_member_groups.group_id, exp_member_groups.can_send_private_messages, exp_member_groups.prv_msg_storage_limit, exp_members.accept_messages
    			...
    			WHERE exp_members.group_id = exp_member_groups.group_id
    			AND exp_member_groups.site_id = '".$this->DB->escape_str($PREFS->ini('site_id'))."'
    			AND exp_members.member_id = '".$this->DB->escape_str($id)."'";
	...
	//----------------------------------------
 	...
 	function _message_data($id, $sender='', $recipient='')
 	{
 		...
 		{
 			$query = $this->DB->query("SELECT message_id, message_folder, message_read FROM exp_message_copies 
 								 WHERE copy_id = '".$this->DB->escape_str($id)."'
 								 AND recipient_id = '".$this->DB->escape_str($recipient)."'");
 		...
 		$sql = "SELECT * FROM exp_message_data WHERE message_id = '".$this->DB->escape_str($mid)."'";
 		
 		if ($sender != '' && is_numeric($sender))
 		{
 			$sql .= " AND sender_id = '".$this->DB->escape_str($sender)."'";
 		}
 		...
 		$results = $this->DB->query("SELECT screen_name FROM exp_members WHERE member_id = '".$this->DB->escape_str($data['sender_id'])."'");
		...
		$data['preview'] = $this->_process_template($this->retrieve_template('preview_message'));
 		...
 		{
 			$results = $this->DB->query("SELECT attachment_name, attachment_id, attachment_size, attachment_hash
 								   FROM exp_message_attachments
 								   WHERE message_id = '".$this->DB->escape_str($mid)."'");
    ...
    function view_message($copy_id = '')
    {
    	...
    	if ($data['message_read'] == 'n')
		{
    		...
    		$this->DB->update('exp_message_copies', $udata, "copy_id = {$copy_id}");
    	}
	...
	function attachment()
	{
		...
		$query = $this->DB->query("SELECT attachment_location, attachment_name, attachment_extension, message_id 
							 FROM exp_message_attachments WHERE attachment_hash = '".$this->DB->escape_str($attach_hash)."'");
		...
		$results = $this->DB->query("SELECT COUNT(*) AS count FROM exp_message_copies 
							   WHERE message_id = '".$this->DB->escape_str($query->row['message_id'])."'
		...
		$this->DB->query("UPDATE exp_message_copies SET attachment_downloaded = 'y' 
					WHERE message_id = '".$this->DB->escape_str($query->row['message_id'])."'
					AND recipient_id = '{$this->member_id}'");
    ...
    function bulletin_board($message='')
    {
   		...
   		$this->DB->query("UPDATE exp_members SET last_view_bulletins = '".$LOC->now."' WHERE member_id = '{$this->member_id}'");
    	...
    	$sql =  "FROM exp_member_bulletin_board b, exp_members m
				 ...
				 AND b.bulletin_group = ".$this->DB->escape_str($SESS->userdata['group_id'])."
		...
		$query = $this->DB->query("SELECT COUNT(b.bulletin_id) AS count ".$sql);
		...
		{
			$this->DB->update('exp_members', array('last_bulletin_date' => $query->row['bulletin_date']), "group_id = '".$this->DB->escape_str($SESS->userdata['group_id'])."'");
		}
    ...
    function delete_bulletin()
    {
   		...
   		if ($this->allegiance == 'cp')
		{
   		...
   		$sql = "SELECT b.sender_id, b.bulletin_id, b.hash
				...
				WHERE b.bulletin_id = '".$this->DB->escape_str($this->cur_id)."'";
		...
		elseif($SESS->userdata['group_id'] != 1 && $query->row['sender_id'] != $SESS->userdata['member_id'])
		{
   		...
   		$this->DB->query("DELETE FROM exp_member_bulletin_board WHERE hash = '".$this->DB->escape_str($query->row['hash'])."'");
    ...
    function send_bulletin($message='')
    {
		...
		$sql = "SELECT group_id, group_title FROM exp_member_groups WHERE site_id = '".$this->DB->escape_str($PREFS->ini('site_id'))."' AND include_in_memberlist = 'y' AND group_id NOT IN ('2', '3', '4') ";
    ...
    function sending_bulletin()
    {
		...
		$sql = "SELECT group_id FROM exp_member_groups WHERE site_id = '".$this->DB->escape_str($PREFS->ini('site_id'))."' AND include_in_memberlist = 'y' AND group_id NOT IN ('2', '3', '4') ";
		...
		foreach($groups as $group_id)
		{
			...
			$this->DB->insert('exp_member_bulletin_board', $data);
			
			if ($LOC->now >= $begins)
			{
				$this->DB->update('exp_members', array('last_bulletin_date' => $LOC->now), "group_id = '".$this->DB->escape_str($group_id)."'");
			}
		}
    ...
    function edit_list()
    {
    	...
    	if ( ! $IN->GBL('toggle', 'POST'))
        {
        	...
        	for ($i=0, $s = sizeof($person); $i < $s; ++$i)
        	{
        		...
        		$this->DB->insert('exp_message_listed', $data);
        		
        		if ($which == 'blocked')
        		{
        			$this->DB->query("DELETE FROM exp_message_copies 
        						WHERE sender_id = '".$this->DB->escape_str($person[$i])."'
        						AND recipient_id = '{$this->member_id}'");
        		}
        	}
        ...
        { 
			if (strstr($key, 'toggle') AND ! is_array($val))
			{
				if ($IN->GBL('daction', 'POST') == 'delete')
				{
					$this->DB->query("DELETE FROM exp_message_listed
        						WHERE member_id = '{$this->member_id}'
        						AND listed_id = '".$this->DB->escape_str($val)."'");
        		}
        	}
        }
    ...
    function maintenance()
    {
    	...
    	$query = $this->DB->query("SELECT copy_id FROM exp_message_copies 
    						 WHERE recipient_id = '{$this->member_id}'
    						 AND message_deleted = 'y'
    	...
    	$query = $this->DB->query("SELECT d.message_id FROM exp_message_data d
    	...
    	if ($query->num_rows > 0)
    	{
    		...
    		if (sizeof($delete) > 0)
    		{
    			$this->DB->query("DELETE FROM exp_message_data WHERE message_id IN ('".implode("','", $delete)."')");
    		}
    	}
 	}
 	/* END */
