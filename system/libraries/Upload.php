...
class CI_Upload {
	...
	var $new_filename = ''; // Èלÿ םמגמדמ פאיכא
	...
	function do_upload($field = 'userfile')
	{
		...
		if ( ! $this->is_allowed_dimensions_small())
		{
			$this->set_error('upload_invalid_dimensions_small');
			return FALSE;
		}
		...
		chmod($this->upload_path.$this->file_name, 0776);
	...
	function set_filename($path, $filename)
	{
		...
		if (empty($this->new_filename))
		{
			...
			if ( ! file_exists($path.$filename) or  $this->overwrite) 
				return $filename;
		}
		else 
		{
			$filename = $this->new_filename;
			if ( ! file_exists($path.$filename.$this->file_ext)  or  $this->overwrite) 
				return $filename.$this->file_ext;
		}
	...
	{
		$this->min_width = ( ! eregi("^[[:digit:]]+$", $n)) ? 0 : $n;
	}
	...
	{
		$this->min_height = ( ! eregi("^[[:digit:]]+$", $n)) ? 0 : $n;
	}
	...
	function is_allowed_dimensions_small()
	{
		if ( ! $this->is_image())
		{
			return TRUE;
	 ...
	 * @return	string
	 */	
	function display_errors($open = '', $close = '<br />')
	{
		$str = '';
