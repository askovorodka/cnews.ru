...
class CI_Image_lib {
	...
	var	$y_axis				= '';
	 ...
	 // Фиксировать размер уменьшеного изображения по стороне. Если width то изображение будет пропорционально уменьшено, ширина будет точно равна заданной, а высота будет пропорциональной. Еще доступен параметр height. Оставьте поле пустым, и система сама выберет более удачный вариант. 
	var $prop_type = 'width';
	...
	function clear()
	{
		$props = array('source_folder', 'dest_folder', 'source_image', 'full_src_path', 'full_dst_path', 'new_image', 'image_type', 'size_str', 'quality', 'orig_width', 'orig_height', 'rotation_angle', 'x_axis', 'y_axis', 'create_fnc', 'copy_fnc', 'wm_overlay_path', 'wm_use_truetype', 'dynamic_output', 'wm_font_size', 'wm_text', 'wm_vrt_alignment', 'wm_hor_alignment', 'wm_padding', 'wm_hor_offset', 'wm_vrt_offset', 'wm_font_color', 'wm_use_drop_shadow', 'wm_shadow_color', 'wm_shadow_distance', 'wm_opacity', "prop_type");
	...
	{	
		...
		if ($action == 'crop')
		{
			...
			*/
			...
			if (empty($this->x_axis))
			{
				$this->x_axis = ($this->orig_width-$this->width)/2;
			}
			
			$ymore = ($this->orig_height-$this->height)/3;
			if (empty($this->y_axis))
			{
				$this->y_axis = $ymore;
			}			
	...
	function image_reproportion()
	{
	   ...
	   // Коэффициент определяет, во сколько раз ширина реальной
	   // картинки превышает максимальный размер
	   $w_coef = 1;
	   // ... высота
	   $h_coef = 1;
	   // Общий
	   ...
	   {
		   $w_coef = $this->width / $this->orig_width;
	   }

	   if ($this->orig_height > $this->height) 
	   {
		   $h_coef = $this->height / $this->orig_height;
	   }

	   // Чтобы картинка была оптимального размера,
	   // выбираем самый маленький из коэффициентов
	   if ($this->prop_type == "width")
	   {
	   	$coef = $w_coef; // Фиксируем пропорции по ширине
	   }
	   elseif ($this->prop_type == "height")
	   {
	   	$coef = $h_coef; // Фиксируем пропорции по высоте
	   }
	   else 
	   {
		   if ($w_coef > $h_coef) $coef = $w_coef;
			 else $coef = $h_coef;
	   }
	
		$new_width	= floor($this->orig_width * $coef);		
		$new_height	= floor($this->orig_height * $coef);
