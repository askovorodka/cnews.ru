...
<div class="content">
	...
	<h2>Список статей обзора &laquo;{$reviews.name}&raquo;</h2>
		...
		{if $articles}
			...
			{foreach from=$articles item=item}
				<tr>
					...
					<td><a href="{$DOMAIN}/admin/reviews_articles/preview/{$item.id}/">смотреть</a></td>
					...
					<td user_id="{$item.users_user_id}">
						<div class="btn-group">
						  ...
						  <ul class="dropdown-menu">
						    <li><a href="{$DOMAIN}/admin/reviews_articles/articles_edit/{$item.id}/"><i class="icon-edit"></i> Редактировать</a></li>
						    <li><a href="{$DOMAIN}/admin/reviews_articles/articles_delete/{$item.id}/" class="articles_delete"><i class="icon-remove"></i> Удалить</a></li>
						  </ul>
						</div>					
					</td>
				</tr>
		...
		<tr>
			<td colspan="9">
				<div class="control-group">
					<button type="button" class="btn" onClick="location=protocol + location.hostname + '/admin/reviews_articles/articles_add/{$reviews.id}/'"><i class="icon-plus"></i> Добавить статью</button>
				</div>
			</td>
		</table>
